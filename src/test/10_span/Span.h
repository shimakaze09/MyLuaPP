...
struct Span_iterator {
  ...
  constexpr reference operator*() const noexcept { return *_Myptr; }

  constexpr pointer operator->() const noexcept { return _Myptr; }
  ...
  constexpr void _Verify_offset(
      [[maybe_unused]] const difference_type _Off) const noexcept {
#ifndef NDEBUG
    if (_Off != 0) {
  ...
  friend constexpr Span_iterator operator+(const difference_type _Off,
                                           Span_iterator _Next) noexcept {
    return _Next += _Off;
  }
  ...
  constexpr difference_type operator-(
      const Span_iterator& _Right) const noexcept {
    assert(_Mybegin == _Right._Mybegin && _Myend == _Right._Myend &&
...
#ifndef NDEBUG
  friend constexpr void _Verify_range(const Span_iterator& _First,
                                      const Span_iterator& _Last) noexcept {
  ...
  [[nodiscard]] constexpr pointer _Unwrapped() const noexcept { return _Myptr; }
  ...
  constexpr void _Seek_to(const pointer _It) noexcept { _Myptr = _It; }
...
struct Span_extent_type {
  ...
  [[nodiscard]] constexpr std::size_t size() const noexcept { return _Extent; }
};
...
struct Span_extent_type<DynamicExtent> {
  ...
  constexpr explicit Span_extent_type(const size_t _Size) noexcept
      : _Mysize(_Size) {}

  [[nodiscard]] constexpr size_t size() const noexcept { return _Mysize; }
...
template <class _Rng>
struct Has_container_interface<
    _Rng, std::void_t<decltype(std::data(std::declval<_Rng&>())),
                      decltype(std::size(std::declval<_Rng&>()))>>
    : std::true_type {};
...
class Span : public Span_extent_type<_Extent> {
 public:
  ...
  template <std::size_t _Ext = _Extent,
            std::enable_if_t<_Ext == 0 || _Ext == DynamicExtent, int> = 0>
  ...
  constexpr Span(pointer _Ptr, size_type _Count) noexcept  // strengthened
    ...
    if constexpr (_Extent != DynamicExtent) {
      assert(_Count == _Extent &&
             "Cannot construct span with static extent from range [ptr, ptr + "
             "count) as count != extent");
    }
#endif  // !NDEBUG
  }
  ...
  constexpr Span(pointer _First, pointer _Last) noexcept  // strengthened
    ...
    if constexpr (_Extent != DynamicExtent) {
      assert(_Last - _First == _Extent &&
             "Cannot construct span with static extent from range [first, "
             "last) as last - first != extent");
    }
#endif  // !NDEBUG
  }
  ...
  template <
      std::size_t _Size,
      std::enable_if_t<_Extent == DynamicExtent || _Extent == _Size, int> = 0>
  constexpr Span(element_type (&_Arr)[_Size]) noexcept
      : _Mybase(_Size), _Mydata(_Arr) {}

  template <
      class _OtherTy, std::size_t _Size,
      std::enable_if_t<
          std::conjunction_v<
              std::bool_constant<_Extent == DynamicExtent || _Extent == _Size>,
              std::is_convertible<_OtherTy (*)[], element_type (*)[]>>,
          int> = 0>
  constexpr Span(std::array<_OtherTy, _Size>& _Arr) noexcept
      : _Mybase(_Size), _Mydata(_Arr.data()) {}
  ...
  template <
      ...
      std::enable_if_t<
          ...
          int> = 0>
  constexpr Span(const std::array<_OtherTy, _Size>& _Arr) noexcept
      : _Mybase(_Size), _Mydata(_Arr.data()) {}

  template <
      class _Rng,
      std::enable_if_t<IsSpan_compatible_range<_Rng, element_type>, int> = 0>
  constexpr Span(_Rng& _Range)
      : _Mybase(static_cast<size_type>(std::size(_Range))),
        _Mydata(std::data(_Range)) {
#ifndef NDEBUG
    if constexpr (_Extent != DynamicExtent) {
      assert(std::size(_Range) == _Extent &&
             "Cannot construct span with static extent from range r as "
             "std::size(r) != extent");
    }
#endif  // !NDEBUG
  }
  ...
  template <class _Rng,
            std::enable_if_t<IsSpan_compatible_range<const _Rng, element_type>,
                             int> = 0>
  ...
  template <class _OtherTy, std::size_t _OtherExtent,
            std::enable_if_t<
                std::conjunction_v<
                    std::bool_constant<_Extent == DynamicExtent ||
                                       _OtherExtent == DynamicExtent ||
  ...
  constexpr Span(const Span<_OtherTy, _OtherExtent>& _Other) noexcept
    ...
    if constexpr (_Extent != DynamicExtent) {
      assert(_Other.size() == _Extent &&
             "Cannot construct span with static extent from other span as "
             "other.size() != extent");
    }
#endif  // !NDEBUG
  }
...
#endif  // !NDEBUG
    return Span<element_type, _Count>{_Mydata + (this->size() - _Count),
                                      _Count};
  }
...
#endif  // !NDEBUG
    return Span<element_type, DynamicExtent>{_Mydata + (this->size() - _Count),
                                             _Count};
  }
  ...
  [[nodiscard]] constexpr auto subspan() const noexcept /* strengthened */ {
    if constexpr (_Extent != DynamicExtent) {
      ...
      static_assert(_Count == DynamicExtent || _Count <= _Extent - _Offset,
                    "Count out of range in span::subspan()");
    }
#ifndef NDEBUG
    else {
      assert(_Offset <= this->size() &&
             "Offset out of range in span::subspan()");

      if constexpr (_Count != DynamicExtent) {
        assert(_Count <= this->size() - _Offset &&
               "Count out of range in span::subspan()");
      }
    }
#endif  // !NDEBUG
    using _ReturnType =
        Span<element_type, _Count != DynamicExtent
                               ? _Count
                               : (_Extent != DynamicExtent ? _Extent - _Offset
                                                           : DynamicExtent)>;
    return _ReturnType{_Mydata + _Offset, _Count == DynamicExtent
                                              ? this->size() - _Offset
                                              : _Count};
  }
  ...
  [[nodiscard]] constexpr auto subspan(
      ...
      const size_type _Count = DynamicExtent) const noexcept
  /* strengthened */ {
#ifndef NDEBUG
    assert(_Offset <= this->size() &&
  ...
  [[nodiscard]] constexpr pointer data() const noexcept { return _Mydata; }
...
auto as_bytes(Span<_Ty, _Extent> _Sp) noexcept {
  using _ReturnType =
      Span<const std::byte,
           _Extent == DynamicExtent ? DynamicExtent : sizeof(_Ty) * _Extent>;
  return _ReturnType{reinterpret_cast<const std::byte*>(_Sp.data()),
                     _Sp.size_bytes()};
}

// FUNCTION TEMPLATE as_writable_bytes
template <class _Ty, std::size_t _Extent,
          std::enable_if_t<!std::is_const_v<_Ty>, int> = 0>
auto as_writable_bytes(Span<_Ty, _Extent> _Sp) noexcept {
  using _ReturnType =
      Span<std::byte,
           _Extent == DynamicExtent ? DynamicExtent : sizeof(_Ty) * _Extent>;
  return _ReturnType{reinterpret_cast<std::byte*>(_Sp.data()),
                     _Sp.size_bytes()};
}
